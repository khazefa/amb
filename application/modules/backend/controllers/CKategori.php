...
{
    private $cname = 'backend/kategori';
    private $cname_admin_cms = 'backend/admin-cms';
    private $view_dir = 'backend/kategori/';
	...
	public function __construct()
	{
		parent::__construct();
		$this->isLoggedIn();
        $this->load->model('backend/Kategori_model','MKategori');
        $this->readonly = FALSE;
    }
    ...
    public function get_list($type){
        ...
        //Condition
        if ($fcode != "") $arrWhere['kategoricode'] = $fcode;
        if ($fname != "") $arrWhere['kategorititle'] = $fname;
        if ($flink != "") $arrWhere['kategoriurl'] = $flink;
        if ($fpublish != "") $arrWhere['kategoripublished'] = $fpublish;
        if ($fposition != "") $arrWhere['priory'] = $fposition;

        $rs = $this->MKategori->get_data($this->security->xss_clean($arrWhere), array('priory'=>'ASC'), $flimit);
        
        switch($type) {
            case "json":
                foreach ($rs as $r) {
                    $pid = filter_var($r['kategoriID'], FILTER_SANITIZE_NUMBER_INT);
                    $code = filter_var($r['kategoricode'], FILTER_SANITIZE_STRING);
                    $name = filter_var($r['kategorititle'], FILTER_SANITIZE_STRING);
                    $link = filter_var($r['kategoriurl'], FILTER_SANITIZE_STRING);
                    $publish = filter_var($r['kategoripublished'], FILTER_SANITIZE_NUMBER_INT);
    ...
    function add()
    {   
        $this->global['pageTitle'] = "Tambah Kategori - ".APP_NAME;
        $this->global['pageMenu'] = 'Tambah Kategori';
        $this->global['contentHeader'] = 'Tambah Kategori';
        $this->global['contentTitle'] = 'Tambah Kategori';
        $this->global ['name'] = $this->accName;
        $this->global ['type'] = $this->accType;
        $this->global ['group'] = $this->accGroup;
        $this->global ['role'] = $this->accRole;
    ...
    function create()
    {
        ...
        $dataInfo = array('kategoricode'=> $code, 'kategoridatecreated'=>$current_date, 'kategoriurl'=>$flink, 
            'kategorititle'=>$fname, 'kategoripublished'=>$fpublish, 'priory'=>0);
        $count = $this->MKategori->check_data_exists(array('kategoricode' => $fcode));
        if ($count > 0)
        { 
    ...
    function edit($fkey = NULL)
    {
        ...
        $this->global['pageTitle'] = "Edit Kategori - ".APP_NAME;
        $this->global['pageMenu'] = 'Edit Kategori';
        $this->global['contentHeader'] = 'Edit Kategori';
        $this->global['contentTitle'] = 'Edit Kategori';
        $this->global ['name'] = $this->accName;
        $this->global ['type'] = $this->accType;
        $this->global ['group'] = $this->accGroup;
        $this->global ['role'] = $this->accRole;
    ...
    public function get_edit($fkey){
        $rs = array(); 
        $arrWhere = array('kategoricode'=>$fkey);
        $flimit = 0;

        $rs = $this->MKategori->get_data($this->security->xss_clean($arrWhere), array(), $flimit);
    ...
    function update()
    {
        ...
        $dataInfo = array('kategoriurl'=>$flink, 'kategorititle'=>$fname, 'kategoripublished'=>$fpublish, 'priory'=>0);

        $result = $this->MKategori->update_data($dataInfo, $fcode);
        if($result == true)
        {
