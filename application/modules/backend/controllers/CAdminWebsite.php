<?php
/**
 * Class CAdminWebsite.php.
 * Desc: Class for every user admin features function purposes
 * @author: Sigit Prayitno
 * @email: cybergitt@gmail.com
 */

class CAdminWebsite extends Back_Controller
{
    private $cname = 'backend/admin-website';
    private $cname_module = 'backend/setting-module';
    private $view_dir = 'backend/asite/';
	...
	{
		$this->global['pageTitle'] = 'Admin Website';
		$this->global['contentHeader'] = 'Admin Website';
		$this->global['contentTitle'] = 'Admin Website';
    ...
    /**
     * This function is used to get list data bidang in array
     */
    private function get_list_bidang(){
        $rs = array();
        $arrWhere = array();
        $data = array();
        $output = null;
        ...
        $rs = $this->MBidang->get_data($this->security->xss_clean($arrWhere), array('bidangsubtitle'=>'ASC'), $flimit);
        foreach ($rs as $r) {
    ...
    /**
     * This function is used to get list data bidang title in array
     */
    private function get_list_title_bidang(){
        $rs = array();
        $arrValue = array();
        $data = array();
        $output = null;
        ...
        $arrValue = array('bidangtitle');
        $rs = $this->MBidang->get_data_distinct($this->security->xss_clean($arrValue), $flimit);
        foreach ($rs as $r) {
            $title = $r['bidangtitle'];

            $row['title'] = filter_var($title, FILTER_SANITIZE_STRING);
            $data[] = $row;
        }
        $output = $data;
        return $output;
    ...
    {   
        if($this->isCMSAdmin()){
            ...
            $data['list_bidang'] = $this->get_list_bidang();
            $data['list_bidang_title'] = $this->get_list_title_bidang();
    ...
    function create()
    {
        ...
        $fname = $this->input->post('fname', TRUE);
        $fkat = $this->input->post('fkat', TRUE);
        $femail = $this->input->post('femail', TRUE);
        ...
        $dataInfo = array('adminname'=>$funame, 'adminpassword'=>md5($fupass), 'adminrealname'=>$fname, 'adminemail'=>$femail, 
            'adminkat'=>$fkat, 'admintipe'=> 'Website', 'admingroup'=>'', 'adminrole'=>'', 'admindropped'=>0, 'adminactivated'=>$factive, 
            'user_media'=>'');
