...
/**
 * Class CMedia.php.
 * Desc: Class for every Media function purposes
 * @author: Sigit Prayitno
 * @email: cybergitt@gmail.com
 */

class CMedia extends Back_Controller
{
    private $cname = 'backend/media';
    private $cname_admin_cms = 'backend/admin-cms';
    private $view_dir = 'backend/media/';
	...
	public function __construct()
	{
		parent::__construct();
		$this->isLoggedIn();
        $this->load->model('backend/Media_model','MMedia');
        $this->readonly = FALSE;
    }
	...
	{
		$this->global['pageTitle'] = 'Media';
		$this->global['contentHeader'] = 'Media';
		$this->global['contentTitle'] = 'Media';
    ...
    public function get_list($type){
        ...
        //Condition
        if ($fcode != "") $arrWhere['mediacode'] = $fcode;
        if ($fname != "") $arrWhere['mediatitle'] = $fname;
        if ($flink != "") $arrWhere['mediaurl'] = $flink;
        if ($ftype != "") $arrWhere['mediatipe'] = $ftype;
        if ($fpublish != "") $arrWhere['mediapublished'] = $fpublish;
        if ($fposition != "") $arrWhere['priory'] = $fposition;

        $rs = $this->MMedia->get_data($this->security->xss_clean($arrWhere), array('priory'=>'ASC'), $flimit);
        
        switch($type) {
            case "json":
                foreach ($rs as $r) {
                    $pid = filter_var($r['mediaID'], FILTER_SANITIZE_NUMBER_INT);
                    $code = filter_var($r['mediacode'], FILTER_SANITIZE_STRING);
                    $name = filter_var($r['mediatitle'], FILTER_SANITIZE_STRING);
                    $type = filter_var($r['mediatipe'], FILTER_SANITIZE_STRING);
                    $link = filter_var($r['mediaurl'], FILTER_SANITIZE_STRING);
                    $publish = filter_var($r['mediapublished'], FILTER_SANITIZE_NUMBER_INT);
    ...
    function add()
    {   
        $this->global['pageTitle'] = "Tambah Media - ".APP_NAME;
        $this->global['pageMenu'] = 'Tambah Media';
        $this->global['contentHeader'] = 'Tambah Media';
        $this->global['contentTitle'] = 'Tambah Media';
        $this->global ['name'] = $this->accName;
        $this->global ['type'] = $this->accType;
        $this->global ['group'] = $this->accGroup;
        $this->global ['role'] = $this->accRole;
    ...
    function create()
    {
        ...
        $dataInfo = array('mediacode'=> $code, 'mediadatecreated'=>$current_date, 'mediaurl'=>$flink, 
            'mediatitle'=>$fname, 'mediatipe'=>$ftype, 'mediapublished'=>$fpublish, 'priory'=>0);
        $count = $this->MMedia->check_data_exists(array('mediacode' => $fcode));
        if ($count > 0)
        { 
        ...
        { 
            $result = $this->MMedia->insert_data($dataInfo);
    ...
    function edit($fkey = NULL)
    {
        ...
        $this->global['pageTitle'] = "Edit Media - ".APP_NAME;
        $this->global['pageMenu'] = 'Edit Media';
        $this->global['contentHeader'] = 'Edit Media';
        $this->global['contentTitle'] = 'Edit Media';
        $this->global ['name'] = $this->accName;
        $this->global ['type'] = $this->accType;
        $this->global ['group'] = $this->accGroup;
        $this->global ['role'] = $this->accRole;
    ...
    public function get_edit($fkey){
        $rs = array(); 
        $arrWhere = array('mediacode'=>$fkey);
        $flimit = 0;

        $rs = $this->MMedia->get_data($this->security->xss_clean($arrWhere), array(), $flimit);
    ...
    function update()
    {
        ...
        $dataInfo = array('mediaurl'=>$flink, 'mediatitle'=>$fname, 'mediatipe'=>$ftype, 'mediapublished'=>$fpublish, 'priory'=>0);

        $result = $this->MMedia->update_data($dataInfo, $fcode);
        if($result == true)
        {
    ...
    {
        $result = $this->MMedia->delete_data($fkey);
