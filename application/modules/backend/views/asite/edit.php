...
$arr_title = array();
$arr_selects = array();
foreach($list_bidang_title as $c){
    array_push($arr_title, $c['title']);
}
foreach($list_bidang as $row){
    if(in_array($row['name'], $arr_title)){
        $arr_selects[$row['name']][] = array('subcode'=>$row['subcode'], 'subname'=>$row['subname']);
    }
}
?>
<!-- Main content -->
                ...
                <input type="hidden" name="fid" id="fid" value="<?php echo $records[0]['id'];?>">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="text" class="form-control" id="funame" name="funame" value="<?php echo $records[0]['uname'];?>" readonly="readonly">
                                <p class="help-block">Nama Pengguna tidak dapat diganti</p>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label">Kata Sandi <span class="text-danger">*</span></label>
                            <div class="col-sm-6">
                                <input type="password" class="form-control" id="fupass" name="fupass" placeholder="Ganti Kata Sandi" pattern=".{4,8}" required title="4 to 8 characters">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="email" class="form-control" id="femail" name="femail" value="<?php echo $records[0]['email'];?>" required="required">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label">Bidang Tujuan Artikel <span class="text-danger">*</span></label>
                            <div class="col-sm-8">
                                <?php
                                    $html = '<select id="fkat" name="fkat" class="selectpicker form-control" data-live-search="true">';
                                    foreach($arr_selects as $key => $value){
                                        if(!empty($value)){
                                            $html .='<optgroup label="'.$key.'">'; 
                                            foreach($value as $k => $v){
                                                if($v['subcode'] === $records[0]['kat']){
                                                    $html .='<option value="'.$v['subcode'].'" selected>'.$v['subname'].'</option>';
                                                }else{
                                                    $html .='<option value="'.$v['subcode'].'">'.$v['subname'].'</option>';
                                                }
                                            }
                                            $html .='</optgroup>'; 
                                        } 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <button type="button" class="btn btn-warning" data-toggle="modal" data-target=".modal-data-modul">Atur (<?= count($access_rights);?> hak akses)</button>
                            </div>
                        </div>
                    </div>
                    <!-- /.box-body -->
...
<div class="modal fade modal-data-modul" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
            ...
            <form class="form-horizontal" action="<?php echo base_url($classname_module.'/insert_rights');?>" method="POST" role="form">
            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>">
            <input type="hidden" name="fuid" id="fuid" value="<?php echo $records[0]['id'];?>">
                <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="col-sm-6 offset-md-6 bg-warning">
                                    <div class="checkbox">
                        ...
                        <div class="row">
                                <div class="col-md-12">
                                    <?php
                                        ...
                                        $data_module_rights = array();
                                        $fcoverage = $access_rights;
                                        if(!empty($fcoverage)){
                                            // $e_fcoverage = explode(';', $fcoverage);
                                            foreach ($fcoverage as $e){
                                                if(!empty($e)){
                                                    array_push($data_module_rights, trim($e));
                                                }
                                            }
                                        }
                                    ?>
                                    ...
                                    <?php
                                        foreach($arr_col1 as $c1){
                                            ...
                                            if (in_array($code, $data_module_rights)){
                                                $checked = "checked";
                                            }
                ...
                <div class="modal-footer">
                    <button type="submit" id="btn_submit_modul" class="btn btn-primary">Simpan</button>
                </div>
            </form>
		</div>
	</div>
