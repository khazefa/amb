...
{
    protected $tbl_bidang = 'bidangartikel';
    protected $primKey = 'bidangID';
    protected $indexKey = 'bidangcode';
    protected $order = array('bidangtitle' => 'asc'); // default order
    ...
    // This function used to get list data distinct by this table only, not join table, with array parameters
    function get_data_distinct($arrValue = array(), $limit = 0){
        $rs = array();
        //Flush Param
        $this->db->flush_cache();
		...
		if(empty($arrValue)){
            ...
            if(is_array($arrValue)){
                $impVal = implode($arrValue, ",");
            }else{
                $impVal = $arrValue;
            }
			$this->db->distinct();
			$this->db->select($impVal);
			$this->db->from($this->tbl_bidang);
    ...
    {
        $this->db->trans_start();
        $this->db->insert($this->tbl_bidang, $dataInfo);
    ...
    {
        $this->db->where($this->primKey, $id);
        $this->db->update($this->tbl_bidang, $dataInfo);
